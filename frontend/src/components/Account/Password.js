  ...
  function Password({setIsEmailVerified, loginPassword, setLoginPassword}) {
    ...
    return (
      ...
      <FormControl variant="floating" id="first-name" width="50%" margin="30px 20px 0 0">
            ...
            <Button colorScheme='blue' variant='ghost' borderRadius="3px" onClick={()=>setIsEmailVerified(false)}>Back</Button>
            
            <Button colorScheme='blue' borderRadius="5px" variant='solid'>Next</Button>
            </Box>
      </FormControl>
      <Checkbox transform="translateY(-150px)" onChange={()=>setPasswordType(!passwordType)}>Show Password</Checkbox>
      </Box>
      </Box>
    )
  }
